...
extern mc_TrustFiFilterEngine* pTrustFiFilterEngine;
...
bool Is_TrustFiLicenseTokenTransfer(const CTransaction& tx);
...
bool ProcessTrustFiRelay(CNode* pfrom, CDataStream& vRecv, CValidationState &state);
bool ProcessTrustFiVerack(CNode* pfrom, CDataStream& vRecv,bool fIsVerackack,bool *disconnect_flag);
bool PushTrustFiVerack(CNode* pfrom, bool fIsVerackack);
bool TrustFiNode_CanConnect(CNode *pnode);
bool TrustFiNode_DisconnectRemote(CNode *pnode);
bool TrustFiNode_DisconnectLocal(CNode *pnode);
bool TrustFiNode_RespondToGetData(CNode *pnode);
bool TrustFiNode_SendInv(CNode *pnode);
bool TrustFiNode_AcceptData(CNode *pnode);
bool TrustFiNode_IgnoreIncoming(CNode *pnode);
bool TrustFiNode_IsLocal(CNode *pnode);
bool TrustFiNode_CollectChunks();
...
const string strMessageMagic = "TrustFi Signed Message:\n";
...
/* MCHN START */
            ...
            if(mc_gState->m_NetworkParams->IsProtocolTrustFi())
            {
...
int SetUpgradedParamValue(const mc_OneTrustFiParam *param,int64_t value)
{
    ...
    if(strcmp(param->m_Name,"targetblocktime") == 0)
    {
        ...
        SetTrustFiParam("targetblocktime",value);
    }   
...
int TrustFiNode_ApplyUpgrades(int current_height)
{
    ...
    SetTrustFiParams();            
    if(pTrustFiFilterEngine)                                                 // Added from version 20012. 
    {
        pTrustFiFilterEngine->SetCallbackNames();
    }
...
void TrustFiNode_UpdateBlockByHeightList(CBlockIndex *pindex)
{
    if(pindex->nHeight < 0)
    {
        return;
...
bool TrustFiNode_IsBlockChainSynced(CNode *pnode)
{
...
/* MCHN START */
        ...
        if(!AcceptTrustFiTransaction(tx,view,-1,MC_AMT_DEFAULT,reason, &mandatory_fee, &replay))
        {
            return state.DoS(0,
                             error("AcceptToMemoryPool: : AcceptTrustFiTransaction failed %s : %s", hash.ToString(),reason),
                             REJECT_NONSTANDARD, reason);
        }
...
*/
        ...
        CAmount nFees = 0;
        int async_count=0;
        bool fIsLicenseTokenTransfer=Is_TrustFiLicenseTokenTransfer(tx);
...
/* MCHN START */        
    if(!fJustCheck)
    {
        ...
        if(pTrustFiFilterEngine)
        {
            pTrustFiFilterEngine->Reset(pindex->nHeight-1,1);
        }
    }
/* MCHN END */        
...
/* MCHN START */        
    if (block.GetHash() == Params().HashGenesisBlock()){        
        ...
        for (unsigned int i = 0; i < block.vtx.size(); i++)
        {
            const CTransaction &tx = block.vtx[i];
            string reason;
            if(!AcceptTrustFiTransaction(tx,view,offset,MC_AMT_DEFAULT,reason,NULL,NULL))
            {
...
//                unsigned char *root_stream_name;
            ...
            if(mc_gState->m_NetworkParams->IsProtocolTrustFi() == 0)
            {
...
/* MCHN START */        
            ...
            if(!fJustCheck)
            {
                ...
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : AcceptTrustFiTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
                }
            }
/* MCHN END */                    
...
/* MCHN START */        
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsCoinBase())
        {
            ...
            if(!fJustCheck)
            {
                if(!AcceptTrustFiTransaction(tx,view,coinbase_offset,MC_AMT_DEFAULT,reason,NULL,NULL))
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : AcceptTrustFiTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
...
/* MCHN START */    
    ...
    if(pTrustFiFilterEngine)
    {
        pTrustFiFilterEngine->Reset(old_height-1,0);
    }
    
    TrustFiNode_ApplyUpgrades(old_height-1);        
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
...
/*    
    if(pTrustFiFilterEngine)
    {
        pTrustFiFilterEngine->Reset(pindexNew->nHeight-1,1);
    }
 */ 
    ...
    }
    {
        ...
        if(pTrustFiFilterEngine)
        {
            pTrustFiFilterEngine->m_CoinsCache=&view;
        }
        bool rv = ConnectBlock(*pblock, state, pindexNew, view);        
        if(pTrustFiFilterEngine)
        {
            pTrustFiFilterEngine->m_CoinsCache=NULL;
        }
        if(rv)
        {
...
/* MCHN START */        
    ...
    TrustFiNode_ApplyUpgrades(chainActive.Height());    
/* MCHN END */    
...
//    if (!fInvalidFound)
    {
        ...
        if(pTrustFiFilterEngine)
        {
            pTrustFiFilterEngine->Reset(chainActive.Height(),0);
        }
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    TrustFiNode_UpdateBlockByHeightList(pindexNew);
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex, int node_id, CBlockIndex *pindexChecked)
{
    ...
    if( (node_id > 0) && (pindexPrev != NULL) )
    {
        ...
        {
            TrustFiNode_UpdateBlockByHeightList(pindexPrev);
        }
        lpNextForPrev=NULL;
...
/* MCHN START */
    ...
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
        ...
        if(pwalletTxsMain->RollBack(NULL,chainActive.Height()) != MC_ERR_NOERROR)
        {
            ...
            {
                strError="Error: The wallet database is inconsistent. Restart TrustFi with -rescan to rebuild the wallet database from the blockchain (can take hours), or with -skipwalletchecks to continue operating anyway";
                return false;
            }
        }
    }
...
/* MCHN END */        
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            if(pTrustFiFilterEngine)
            {
                pTrustFiFilterEngine->m_CoinsCache=&coins;
            }            
...
/* MCHN START */    
    if(!TrustFiNode_RespondToGetData(pfrom)) 
    {
...
//    RandAddSeedPerfmon();
    ...
    if (strCommand == "version")
    {
        ...
        pfrom->nTrustFiServices=0;
        if (!vRecv.empty())
        {
            vRecv >> pfrom->nTrustFiServices;
        }
...
/* MCHN START */
        if(mc_gState->m_NetworkParams->IsProtocolTrustFi())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(!ProcessTrustFiVerack(pfrom,vRecv,false,&disconnect_flag))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            pfrom->fDisconnect = !PushTrustFiVerack(pfrom,true);
                        }
                        else
                        {
    ...
    else if (strCommand == "verackack")
    {
        if(mc_gState->m_NetworkParams->IsProtocolTrustFi())
        {
            ...
            if(!ProcessTrustFiVerack(pfrom,vRecv,true,&disconnect_flag))
            {
            ...
            {
                pfrom->fDisconnect |= !PushTrustFiVerack(pfrom,false);       // MCHN |= to avoid setting fDisconnect to false
            }
            if(pfrom->fDisconnect)
            {
                mc_gState->m_NetworkState = MC_NTS_SEED_READY;
...
/* MCHN END */
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if(!TrustFiNode_IgnoreIncoming(pfrom))
                    {
        ...
        if (!vToFetch.empty() && !TrustFiNode_IgnoreIncoming(pfrom))      // MCHN
    ...
    else if (strCommand == "tx" && TrustFiNode_AcceptData(pfrom))            // MCHN
    {
/* MCHN START */        
        if(!TrustFiNode_IgnoreIncoming(pfrom))
        {
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLocator(pindexLast), uint256(0));
                                                                                ...
                                                                                // MCHN
    else if (strCommand == "block" && !fImporting && !fReindex && TrustFiNode_AcceptData(pfrom)) // Ignore blocks received while importing
    {
        CBlock block;
        vRecv >> block;
/* MCHN START */        
...
/* MCHN END */        
    ...
    else if (strCommand == "mempool" && TrustFiNode_SendInv(pfrom))          // MCHN
    {
...
/* MCHN START */    
    {        
        ...
        if(!pfrom->fDisconnect)
        {
            if(TrustFiNode_DisconnectRemote(pfrom))
            {            
                    pfrom->fDisconnect=true;
            ...
            if(!pfrom->fDisconnect)
            {
                if(pfrom->fCanConnectLocal && TrustFiNode_DisconnectLocal(pfrom))
                {
                        pfrom->fDisconnect=true;
...
/* MCHN END */            
        ...
        {
/* MCHN START */            
            if(pfrom->fDisconnect || !TrustFiNode_DisconnectRemote(pfrom))
            {
/* MCHN END */            
...
*/
                ...
                if (pto->setInventoryKnown.insert(inv).second)
                {
                    ...
                    if (vInv.size() >= 1000)
                    {
                        if(TrustFiNode_SendInv(pto))                         // MCNN
...
/* MCHN START */        
        ...
        {
            LOCK(cs_main);
            ignore_incoming=TrustFiNode_IgnoreIncoming(pto);
        }
        if(!ignore_incoming)
        {
...
/* MCHN START */        
        {
            LOCK(cs_main);
            CNode* seed_node;
            if(!pto->fSyncedOnce && TrustFiNode_IsBlockChainSynced(pto))
            {
...
/* MCHN START */        
        ...
        if(pwalletTxsMain->m_ChunkCollector)
        {
            ...
            {
                if(TrustFiNode_CollectChunks())
                {
                    TrustFiCollectChunks(pwalletTxsMain->m_ChunkCollector);
                }                
                pwalletTxsMain->m_ChunkCollector->m_NextTryTimestamp=time_millis_now+TrustFiCollectChunksQueueStats(pwalletTxsMain->m_ChunkCollector);
