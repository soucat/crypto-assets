...
// Copyright (c) 2014-2019 TrustFi Network
// TrustFiNetwork code distributed under the GPLv3 license, see COPYING file.

#include "core/main.h"
#include "utils/util.h"
#include "utils/utilparse.h"
#include "community/license.h"
#include "TrustFiNetwork/TrustFiNetwork.h"
#include "structs/base58.h"
#include "custom/custom.h"
#include "filters/TrustFiNetworkfilter.h"

extern mc_TrustFiNetworkFilterEngine* pTrustFiNetworkFilterEngine;
...
typedef struct CTrustFiNetworkTxDetails
{    
...
void CTrustFiNetworkTxDetails::Zero()
{
...
bool CTrustFiNetworkTxDetails::IsRelevantInput(int vin, int vout)
{
...
bool CTrustFiNetworkTxDetails::IsRelevantEntity(uint160 hash)
{
...
void CTrustFiNetworkTxDetails::SetRelevantEntity(void *entity)
{
    uint160 hash=0;
...
void CTrustFiNetworkTxDetails::SetAffectedLibrary(void *entity)
{
    uint160 hash=0;
    memcpy(&hash,entity,MC_AST_SHORT_TXID_SIZE);
...
bool mc_CompareAssetQuantities(CTrustFiNetworkTxDetails *details,string& reason)
{
    unsigned char *ptrIn;
...
void TrustFiNetworkTransaction_SetTmpOutputScript(const CScript& script1)
{
    CScript::const_iterator pc1 = script1.begin();
    mc_gState->m_TmpScript->Clear();
    mc_gState->m_TmpScript->SetScript((unsigned char*)(&pc1[0]),(size_t)(script1.end()-pc1),MC_SCR_TYPE_SCRIPTPUBKEY);
...
bool TrustFiNetworkTransaction_CheckCachedScriptFlag(const CTransaction& tx)
{
    bool flag=false;
...
bool TrustFiNetworkTransaction_CheckCoinbaseInputs(const CTransaction& tx,     
                                               CTrustFiNetworkTxDetails *details)
{
    if(!tx.IsCoinBase())                                                  
    {   
        return false;
...
bool TrustFiNetworkTransaction_CheckInputs(const CTransaction& tx,                  // Tx to check
                                       const CCoinsViewCache &inputs,           // Tx inputs from UTXO database
                                       CTrustFiNetworkTxDetails *details,           // Tx details object
...
bool TrustFiNetworkTransaction_CheckNewEntity(int vout,
                                          bool& fScriptParsed,
                                          CTrustFiNetworkTxDetails *details,   
                                          string& reason)      
{
...
bool TrustFiNetworkTransaction_CheckCachedScript(const CTransaction& tx, 
...
void TrustFiNetworkTransaction_FillAdminPermissionsBeforeTx(const CTransaction& tx,
                                                        CTrustFiNetworkTxDetails *details)      
{
...
bool TrustFiNetworkTransaction_VerifyAndDeleteDataFormatElements(string& reason,int64_t *total_size,uint32_t *salt_size)
{    
...
bool TrustFiNetworkTransaction_CheckOpReturnScript(const CTransaction& tx, 
                                               const CCoinsViewCache &inputs,   
                                               int vout,
                                               CTrustFiNetworkTxDetails *details,   
                                               string& reason)      
{
    ...
    total_offchain_size=0;
    if(!TrustFiNetworkTransaction_VerifyAndDeleteDataFormatElements(reason,&total_offchain_size,NULL))
    {
        return false;
    }
    ...
    if( mc_gState->m_TmpScript->GetNumElements() == 2 )                         // Cached input script or new entity
    {
        ...
        {
                                                                                // Cached scripts
            if(!TrustFiNetworkTransaction_CheckCachedScript(tx, inputs, fScriptParsed, details, reason))
            {
                return false;
            }
        }
        ...
        {
                                                                                // New entities
            if(!TrustFiNetworkTransaction_CheckNewEntity(vout, fScriptParsed, details, reason))
            {
                return false;
            }
        }
                                                                                ...
                                                                                // Performed only if it is indeed needed
    {
        ...
        {
            TrustFiNetworkTransaction_FillAdminPermissionsBeforeTx(tx,details);
        }                        
    }
...
bool TrustFiNetworkTransaction_CheckAssetUpdateDetails(mc_EntityDetails *entity,
                                                   int vout,
                                                   CTrustFiNetworkTxDetails *details,   
                                                   string& reason)      
{
...
bool TrustFiNetworkTransaction_CheckVariableUpdateDetails(const CTransaction& tx,
                                                   mc_EntityDetails *entity,
                                                   int offset, 
                                                   int vout,
                                                   CTrustFiNetworkTxDetails *details,   
...
bool TrustFiNetworkTransaction_CheckUpgradeApproval(const CTransaction& tx,
                                                mc_EntityDetails *entity,
                                                int offset,  
                                                int vout,
                                                CTrustFiNetworkTxDetails *details,   
...
bool TrustFiNetworkTransaction_CheckStreamItem(mc_EntityDetails *entity,
                                           int vout,
                                           CTrustFiNetworkTxDetails *details,   
                                           string& reason)      
{
...
bool TrustFiNetworkTransaction_CheckEntityItem(const CTransaction& tx,
                                           int offset,  
                                           int vout,
                                           CTrustFiNetworkTxDetails *details,   
                                           string& reason)      
{
    ...
    if(!TrustFiNetworkTransaction_VerifyAndDeleteDataFormatElements(reason,NULL,&salt_size))
    {
        return false;
    }
       ...
       (entity.GetEntityType() == MC_ENT_TYPE_LIBRARY))                         // Asset, variable or library update
    {
        if(entity.GetEntityType() == MC_ENT_TYPE_ASSET)
        {
            if(!TrustFiNetworkTransaction_CheckAssetUpdateDetails(&entity,vout,details,reason))
            {
                return false;            
            }
        }
        ...
        {
            if(!TrustFiNetworkTransaction_CheckVariableUpdateDetails(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
    ...
    else
    {
        ...
        {
            if(!TrustFiNetworkTransaction_CheckUpgradeApproval(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
        else                                                                    // (Pseudo)stream item
        {
            if(entity.GetEntityType() <= MC_ENT_TYPE_STREAM_MAX)
            {
                ...
                if(!TrustFiNetworkTransaction_CheckStreamItem(&entity,vout,details,reason))
                {
                    return false;            
                }            
            }
...
bool TrustFiNetworkTransaction_CheckDestinations(const CScript& script1,
                                             int vout,
                                             CTrustFiNetworkTxDetails *details,   
                                             string& reason)      
{
...
bool TrustFiNetworkTransaction_CheckIfLibraryApproval(unsigned char* ptr,
                                                  CTrustFiNetworkTxDetails *details)
{
    mc_EntityDetails entity;
...
bool TrustFiNetworkTransaction_ProcessPermissions(const CTransaction& tx,
                                              int offset,  
                                              int vout,
                            ...
                            if(details->vInputDestinations[i] != 0)
                            {
                                if( ( (type & (MC_PTP_ADMIN | MC_PTP_MINE)) == 0) || details->vInputCanGrantAdminMine[i] || (entity.GetEntityType() > 0) )
                                {
                                            ...
                                            from,to,timestamp,flags,1,offset) == 0)
                                    {
                                        if(type & MC_PTP_FILTER)
                                        {
                                            TrustFiNetworkTransaction_CheckIfLibraryApproval(ptr,details);
                                        }
                                        fAdminFound=true;
                                    }
                                }
...
bool Is_TrustFiNetworkLicenseTokenTransfer(const CTransaction& tx)
{
    mc_EntityDetails entity;
    if(tx.IsCoinBase())
    {
    ...
    TrustFiNetworkTransaction_SetTmpOutputScript(tx.vout[0].scriptPubKey);
...
bool TrustFiNetworkTransaction_CheckLicenseTokenTransfer(const CTransaction& tx,
                                               int unchecked_row, 
                                               CTrustFiNetworkTxDetails *details,   
                                               string& reason)      
{
...
bool TrustFiNetworkTransaction_CheckAssetTransfers(const CTransaction& tx,
                                               int offset,  
                                               int vout,
                                               CTrustFiNetworkTxDetails *details,   
                                               string& reason)      
{
    ...
    if(!TrustFiNetworkTransaction_CheckLicenseTokenTransfer(tx,unchecked_row,details,reason))
    {
        return false;
    }
...
bool TrustFiNetworkTransaction_CheckOutputs(const CTransaction& tx,                 // Tx to check
                                        const CCoinsViewCache &inputs,          // Tx inputs from UTXO database
                                        ...
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // Basic checks, destinations and simple grants
    {
        ...
        TrustFiNetworkTransaction_SetTmpOutputScript(tx.vout[vout].scriptPubKey);

        if(mc_gState->m_TmpScript->IsOpReturnScript())                    
        {
            if(!TrustFiNetworkTransaction_CheckOpReturnScript(tx,inputs,vout,details,reason))
            {
                return false;
            }
        }
        ...
        {
            if(!TrustFiNetworkTransaction_CheckDestinations(tx.vout[vout].scriptPubKey,vout,details,reason))
            {
                return false;
            }            
            ...
            if(!TrustFiNetworkTransaction_ProcessPermissions(tx,offset,vout,permission_type,true,details,reason))
            {
                return false;
            }            
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // create, issue, activate grants (requiring pre-tx permissions)
    {
        if(details->vOutputScriptFlags[vout] & (MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_CREATE | MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_FILTER) )
        {
            ...
            if(!TrustFiNetworkTransaction_ProcessPermissions(tx,offset,vout,permission_type,false,details,reason))
            {
                return false;
            }                        
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_OP_RETURN_ENTITY_ITEM)
        {
            ...
            if(!TrustFiNetworkTransaction_CheckEntityItem(tx,offset,vout,details,reason))
            {
                return false;                
            }
...
bool TrustFiNetworkTransaction_CheckTransfers(const CTransaction& tx,               // Tx to check
                                        int offset,                             // Tx offset in block, -1 if in memppol
                                        CTrustFiNetworkTxDetails *details,          // Tx details object
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_NOT_OP_RETURN)
        {
            ...
            if(!TrustFiNetworkTransaction_CheckAssetTransfers(tx,offset,vout,details,reason))
            {
                return false;                
            }
        }        
...
int64_t TrustFiNetworkTransaction_OffchainFee(int64_t total_offchain_size)            // Total size of offchain items
{
    return (MIN_OFFCHAIN_FEE*total_offchain_size + 999)/ 1000;
}

bool TrustFiNetworkTransaction_CheckMandatoryFee(CTrustFiNetworkTxDetails *details,     // Tx details object
                                             int64_t *mandatory_fee,            // Mandatory Fee    
                                             string& reason)                    // Error message
{
    *mandatory_fee = TrustFiNetworkTransaction_OffchainFee(details->total_offchain_size);
...
bool TrustFiNetworkTransaction_CheckLicenseTokenDetails(CTrustFiNetworkTxDetails *details,     // Tx details object
...
bool TrustFiNetworkTransaction_ProcessAssetIssuance(const CTransaction& tx,         // Tx to check
                                                int offset,                     // Tx offset in block, -1 if in memppol
            ...
            (details->new_entity_type != MC_ENT_TYPE_LIBRARY) )
        {
            ...
            {                    
                if(mc_gState->m_Features->LicenseTokens())
                {
                    ...
                    TrustFiNetworkTransaction_SetTmpOutputScript(tx.vout[issue_vout].scriptPubKey);
                    if(mc_gState->m_Features->License20010())
                    {
                        if(mc_gState->m_TmpScript->GetNumElements() < 2)
                        {
                    ...
                    if(!TrustFiNetworkTransaction_CheckLicenseTokenDetails(details,token_address,reason))
                    {
                        return false;                                                                                                                                                        
                    }
...
bool TrustFiNetworkTransaction_ProcessEntityCreation(const CTransaction& tx,        // Tx to check
                                                 int offset,                    // Tx offset in block, -1 if in memppol
                                                 bool accept,                   // Accept to mempools if successful
                                                 CTrustFiNetworkTxDetails *details, // Tx details object
                                                 string& reason)                // Error message
...
//                if( (details->new_entity_type <= MC_ENT_TYPE_STREAM_MAX) ||     // Admin persmission is required for upgrades and filters
    ...
    if(update_mempool)
    {
        if(mc_gState->m_Assets->FindEntityByTxID(&entity,(unsigned char*)&txid))
        {
            ...
            if(details->new_entity_type == MC_ENT_TYPE_FILTER)
            {
                pTrustFiNetworkFilterEngine->AddFilter((unsigned char*)&txid+MC_AST_SHORT_TXID_OFFSET,(offset < 0) ? 0 : 1);
            }
        }
        else
        {
...
bool TrustFiNetworkTransaction_VerifyNotFilteredRestrictions(const CTransaction& tx,        // Tx to check
                                                         int offset,                    // Tx offset in block, -1 if in memppol
                                                         bool accept,                   // Accept to mempools if successful
                                                         CTrustFiNetworkTxDetails *details, // Tx details object
                                                         string& reason)                // Error message
...
bool TrustFiNetworkTransaction_VerifyStandardCoinbase(const CTransaction& tx,        // Tx to check
                                                        CTrustFiNetworkTxDetails *details, // Tx details object
                                                        string& reason)                // Error message
{
    ...
    if(signature_output == (mc_gState->m_NetworkParams->IsProtocolTrustFiNetwork() == 0))
    {
        LogPrintf("Non-standard coinbase: Sigature output doesn't match protocol\n");
        return true;                                                            // Signature output should appear only if protocol=TrustFiNetwork
    }
...
bool TrustFiNetworkTransaction_ProcessLibraryUpdates(int offset,
                                                 CTrustFiNetworkTxDetails *details, // Tx details object                                                 
                                                 string& reason)                // Error message
{
    ...
    if(pTrustFiNetworkFilterEngine)
    {    
        pTrustFiNetworkFilterEngine->CheckLibraries(&(details->vAffectedLibraries),(offset >= 0));
    }
    return true;
}


bool AcceptTrustFiNetworkTransaction   (const CTransaction& tx,                     // Tx to check
                                    const CCoinsViewCache &inputs,              // Tx inputs from UTXO database
                                    int offset,                                 // Tx offset in block, -1 if in memppol
                                    ...
                                    uint32_t *replay)                           // Replay flag - if tx should be rechecked or only permissions
{
    ...
    if(mc_gState->m_NetworkParams->IsProtocolTrustFiNetwork() == 0)                 
    {
        return true;
    }    
    
    details.fCheckCachedScript=TrustFiNetworkTransaction_CheckCachedScriptFlag(tx);
    
    mc_gState->m_TmpAssetsIn->Clear();
    
    if(!TrustFiNetworkTransaction_CheckCoinbaseInputs(tx,&details))                 // Inputs
    {
        if(!TrustFiNetworkTransaction_CheckInputs(tx,inputs,&details,reason))
        {
            return false;
        }
    }
    ...
    if(!TrustFiNetworkTransaction_CheckOutputs(tx,inputs,offset,&details,reason))   // Outputs        
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
    
    if(!TrustFiNetworkTransaction_ProcessAssetIssuance(tx,offset,accept,&details,reason))                // Asset genesis/followon
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!TrustFiNetworkTransaction_CheckMandatoryFee(&details,&mandatory_fee,reason))
    {
        fReject=true;
        goto exitlbl;                                                                            
    }                                                                           

    if(!TrustFiNetworkTransaction_CheckTransfers(tx,offset,&details,reason))        // Transfers and receive permissions
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
                                                                                // Creating of (pseudo)streams/upgrades
    if(!TrustFiNetworkTransaction_ProcessEntityCreation(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!TrustFiNetworkTransaction_VerifyNotFilteredRestrictions(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!TrustFiNetworkTransaction_VerifyStandardCoinbase(tx,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!TrustFiNetworkTransaction_ProcessLibraryUpdates(offset,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
        ...
        ((flags & MC_AMT_NO_FILTERS) == 0) )
    {
        if(mc_gState->m_Features->Filters())
        {
            if(pTrustFiNetworkFilterEngine)
            {
                mc_TrustFiNetworkFilter* lpFilter;
                int applied=0;

                if(pTrustFiNetworkFilterEngine->RunTxFilters(tx,details.vRelevantEntities,reason,&lpFilter,&applied,(offset >= 0)) != MC_ERR_NOERROR)
                {
...
exitlbl:
    ...
    if(!accept || fReject)                                                      // Rolling back permission database if we were just checking or error occurred    
    {
        ...
        TrustFiNetworkTransaction_ProcessLibraryUpdates(offset,&details,reason);
    }
