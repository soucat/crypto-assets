...
uint32_t TrustFiNetworkNextChunkQueryAttempt(uint32_t attempts)
{
...
bool TrustFiNetworkProcessChunkResponse(const CRelayResponsePair *response_pair,map <int,int>* request_pairs,mc_ChunkCollector* collector)
{
...
int TrustFiNetworkResponseScore(mc_RelayResponse *response,mc_ChunkCollectorRow *collect_row,map<int64_t,int64_t>& destination_loads,uint32_t max_total_size)
{
    unsigned char *ptr;
    unsigned char *ptrEnd;
    unsigned char *ptrStart;
...
int TrustFiNetworkCollectChunks(mc_ChunkCollector* collector)
{
...
//                    printf("coll new rsp: row: %d, id: %s, %d\n",row,collect_row->m_State.m_Request.ToString().c_str(),collect_row->m_State.m_RequestPos);
    ...
    {
        TrustFiNetworkProcessChunkResponse(&(item.first),&(item.second.m_Pairs),collector);
            ...
            if(collect_row->m_State.m_Request.IsZero())
            {
                ...
                if(!collect_row->m_State.m_Query.IsZero())
                {
                    ...
                    if(query == NULL)
                    {
                        ...
                        collect_row->m_State.m_QueryNextAttempt=time_now+TrustFiNetworkNextChunkQueryAttempt(collect_row->m_State.m_QueryAttempts);                                                
                ...
                if(query)
                {
                    ...
                    {
                        this_score=TrustFiNetworkResponseScore(&(query->m_Responses[i]),collect_row,destination_loads,max_total_destination_size);
...
int TrustFiNetworkCollectChunksQueueStats(mc_ChunkCollector* collector)
{
    int row;
    int delay;
    mc_ChunkCollectorRow *collect_row;
...
bool TrustFiNetworkRelayResponse(uint32_t msg_type_stored, CNode *pto_stored,
...
//    printf("recv: %d, from: %d, to: %d, hc: %d, size: %d, ts: %u, nc: %u\n",msg_type_in,pfrom->GetId(),pto_stored ? pto_stored->GetId() : 0,hop_count,(int)vPayloadIn.size(),timestamp_received,nonce_received);
    ...
    else
    {
        ...
        {
            if(TrustFiNetworkRelayResponse(msg_type_stored,pto_stored,
