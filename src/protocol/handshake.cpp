...
bool TrustFiNetworkNode_CanConnect(CNode *pnode)
{    
    bool ret=true;
...
bool TrustFiNetworkNode_DisconnectRemote(CNode *pnode)
{
    ...
    {
        return !TrustFiNetworkNode_CanConnect(pnode);
    }
    
    return false;
}

bool TrustFiNetworkNode_DisconnectLocal(CNode *pnode)
{
    if(pnode->fDisconnect)
    {
        return true;
...
bool TrustFiNetworkNode_RespondToGetData(CNode *pnode)
{
    return !(pnode->fDisconnect) && (mc_gState->m_Permissions->CanConnect(NULL,pnode->kAddrRemote.begin()) != 0);    
}

bool TrustFiNetworkNode_SendInv(CNode *pnode)
{
    return !(pnode->fDisconnect) && (mc_gState->m_Permissions->CanConnect(NULL,pnode->kAddrRemote.begin()) != 0);    
}

bool TrustFiNetworkNode_AcceptData(CNode *pnode)
{
    if(mc_gState->m_pSeedNode == (void*)pnode)
    {
        return true;
    ...
    return !TrustFiNetworkNode_DisconnectRemote(pnode);
}

bool TrustFiNetworkNode_IgnoreIncoming(CNode *pnode)
{
...
bool TrustFiNetworkNode_CollectChunks()
{
...
bool TrustFiNetworkNode_IsLocal(CNode *pnode)
{
    return (IsLocal(pnode->addr) || pnode->addr.IsRFC1918()) && (pnode->addr.GetPort() == GetListenPort());
}


bool VerifyTrustFiNetworkVerackHash(string sParameterSetHash,uint64_t nNonce)
{
...
bool ProcessTrustFiNetworkVerack(CNode* pfrom, CDataStream& vRecv,bool fIsVerackack,bool *disconnect_flag)
{   
    ...
    {
        if(!VerifyTrustFiNetworkVerackHash(sParameterSetHash,nNonce))
        {
            if(!fIsVerackack)
            {
...
//                return false;
    ...
    if(MCP_ANYONE_CAN_CONNECT == 0)
    {
        ...
        if(fIsVerackack)
        {
            ...
            if(!TrustFiNetworkNode_CanConnect(pfrom))
            {
...
bool PushTrustFiNetworkVerack(CNode* pfrom, bool fIsVerackack)
{
