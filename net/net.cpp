...
uint64_t nLocalTrustFiNetworkServices = 0;
...
/* MCHN START */
    ...
    nLocalTrustFiNetworkServices |= pEF->NET_Services();
    if(mc_gState->m_NetworkParams->IsProtocolTrustFiNetwork())
    {
        subver=FormatSubVersion("TrustFiNetwork", mc_gState->GetProtocolVersion(), std::vector<string>());
    }
    PushMessage("version", PROTOCOL_VERSION, nLocalServices, nTime, addrYou, addrMe,
                nLocalHostNonce, subver, nBestHeight, true, nLocalTrustFiNetworkServices);
/* MCHN END */
}
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "TrustFiNetwork " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. TrustFiNetwork Core is probably already running."), addrBind.ToString());
        else
